...
export const blogpostRouter = createTRPCRouter({
	...
	update: protectedProcedure
		.input(z.object({ id: z.string(), title: z.string().min(1).optional(), content: z.string().min(1).optional() }))
		.mutation(async ({ ctx, input }) => {
			const db = ctx.db as any;

			const existing = await db.blogPost.findUnique({ where: { id: input.id }, select: { authorId: true } });
			if (!existing) throw new TRPCError({ code: "NOT_FOUND", message: "Post not found" });
			if (existing.authorId !== ctx.session.user.id) throw new TRPCError({ code: "FORBIDDEN", message: "You can only edit your own posts" });

			const data: any = {};
			if (typeof input.title === "string") data.title = input.title;
			if (typeof input.content === "string") data.content = input.content;

			const updated = await db.blogPost.update({ where: { id: input.id }, data });
			return updated;
		}),
