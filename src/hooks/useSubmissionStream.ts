...
export function useSubmissionStream(refetchSubmissions: () => void) {
  ...
  const getTypedResponse = <T extends keyof ResponseTypeMap>(
    status: T
  ): ResponseTypeMap[T] | null => {
    if (metaStatus === status) {
      return metaResponse as ResponseTypeMap[T];
    }
    return null;
  };

  const handleStreamError = useCallback((error: unknown): void => {
  ...
  const processEvent = useCallback((event: string): void => {
  ...
  const processEventStream = useCallback(async (reader: ReadableStreamDefaultReader<Uint8Array>): Promise<void> => {
